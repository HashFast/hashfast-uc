        ...
        */
        ...
        if ((sr & mask) & AVR32_TWI_SR_TXRDY_MASK) {
            if (sr & AVR32_TWI_SR_NACK_MASK) {
                /* done writing */
#if 0
                /* Atmel's ASF driver does this, but then that doesn't mean
                   much because their stuff in general is littered with bugs.
...
void twiInit(void) {
    ...
    twiReset();
    interruptsRegister(twiInterrupt, AVR32_TWI_IRQ, CONFIG_INT_LEVEL_TWI);
}
...
void twiConfig(const twiConfigT *config) {
    ...
    if (config->master != twi.modeMaster)
        twiReset();
    if (config->freq) {
        divisor = (CONFIG_PBA_FREQ + 2 * config->freq - 1) /
                  (2 * config->freq);
        if (divisor > 4)
            divisor -= 4;
        else
