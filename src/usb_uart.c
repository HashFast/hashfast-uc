...
void CheckUsbIncoming(void)
    ...
    if (tx_buffers_used >= TX_BUFFERS-2)          // Flow control back to USB host
        return;
    ...
    if (looking_for_preamble)
        ...
        if (info->resend_op_config == true)
            ...
            if (info->gwq_enabled)
                {
                // Send twice to guard against transmission errors losing one. Right now status messages are
                // turned off, so gwq is at a standstill.  Watchdog will kick in if we lose both.
                ...
                if (ucinfo.mixed_reference_clocks)
                    ...
                    for (i = 0; i < ucinfo.die_count; i++)
                        ...
                        if (module_ref_clocks[i] != ucinfo.ref_frequency)
                            ...
                            // Else we're outa luck, just have to deal with messed up timing until next work restart
                            // Shouldn't be a problem, plenty of tx buffers at the end of a work restart, right?
                            }
                        }
                    }
                gwq_work_restart_complete();
                }
            // else it was a ums_clock_change() and OP_CONFIG's are the user's responsibility
            return;
            }
        ...
        if (info->work_restart_requested || info->restart_phase)
            return;                             // Don't accept more work until restart completed
#endif
        }

    // If we are doing an initialization cycle, IGNORE everything the host sends us
            ...
            *t++ = (uint8_t)ch;
            ...
            if (header_byte == 7)
                ...
                if (crc8 != ch)
                    ...
                    // cgminer will periodically send OP_PING's at us while work is starved due to trouble
                    // communicating with the pool server. That can be used to keep the watchdog at bay.
                ...
                if (h->operation_code == OP_HASH)
                    ...
                    if (h->chip_address == 254)
                        ...
                        if (ucinfo.ntime_roll_total <= 1)
                            ...
                            for (i=0; i<4; i++)                 // Add the silly spare3 field which is
                                {                               // kept out of the USB i/f to keep the
                                *t++ = (uint8_t)0;              // OP_HASH header+data within 64 bytes
                                }
                            }
                        else
                            {
                    ...
                    h->data_length = U32SIZE(struct hf_hash_serial);    // Adjust length and re-compute header crc
                    h->crc8 = hf_crc8((uint8_t *)h);                    // Fix header
                    }
...
void CheckUartOutgoing()
    ...
    irq = cpu_irq_save();
    if (tx_buffers_used > 0 && !dma_tx_in_progress &&
        (pdca_get_channel_status(DMA_UART_TX_CHANNEL) == false))
        {
        cpu_irq_restore(irq);
        ...
        len = 8;
        if (h->data_length)
            len += (h->data_length * 4);                    // Don't include CRC-32 yet - might be USB operation

        switch (h->operation_code)
            {
            // The first thing a host sends to us over a USB link, to initialize communications
            case OP_POWER:                                      // For diagnostic use
                if (le16_to_cpu(h->hdata) & DIAGNOSTIC_POWER_OFF)
                    ...
                    if (system_on())
                        ...
                        power_up_only = true;
                        // Fall through (I know, it's a horrible programming practise)
                    }
            ...
            case OP_CHARACTERIZE:                               // Same initial actions occur for characterization
...
flush_all:
                ...
                rx_head = &uart_rx_buffer[0];               // XXX Disable receiving, could be in the
                rx_tail = &uart_rx_buffer[0];               // XXX middle of packet DMA even...
                rx_buffers_used = 0;                        // Flush EVERYTHING in the receive queue

                info->connected = 1;
                cpu_irq_restore(irq);
                memset(uart_tx_buffer, 0, sizeof(uart_tx_buffer));      // Stand on any old uart_sendinfo data

                usb_powerup_request();                      // Power up if we are not
                ACTIVITY_LED_ON;
                led_mode = LED_STATIC;
                return;
                break;
            ...
            case OP_WORK_RESTART:
                if (info->work_restart_in_progress == false && info->gwq_enabled == true && (len = gwq_work_restart(h)) != 0)
                    {
                    // gwq_work_restart() actually returns 2 packets, an OP_CONFIG and an OP_ABORT
                    // They both get sent (back-to-back). The OP_CONFIG stops periodic status reports,
                    // and instead switches to status triggered on cores going idle. This is put back
                    ...
                    {
                    // UMS mode - normally won't return a packet so we'll toss the packet
                    // Normal gwq engine will do the requested clock change
                    ucinfo.work_restart_requested = false;
                    }
                else
                    {
                    // Toss the request - we already have a work restart going on, or there is no work active, or it's ums
                    ucinfo.work_restart_requested = false;
            ...
            case OP_ABORT:
                if (h->data_length)
                    {
                    // Multiple back-to-back OP_ABORT's, crc8 correct in 1st one once data_length cleared
                    len = h->data_length*sizeof(struct hf_header);  // h->data_length is # of back-to-back aborts
                    h->data_length = 0;
                    }
                break;
#endif
            ...
            // Enter the boot loader. Might need some more protections here....
            case OP_DFU:
        ...
        if (loopback)
            ...
            dma_tx_in_progress++;
            cpu_irq_restore(irq);
            mh = h;                                         // Initialize multiple header pointer
...
void usb_bus_reset(void) {
    ...
    /* note that bus reset is a normal part of USB enumeration, so do not
       take action on it without qualifying it in some way, such as the cdc
       interface having been configured (which is what usb_iface_cfg
       indicates) */
    if (usb_iface_cfg)
        usb_powerdown_request();
}
...
static iram_size_t usb_write_space_available;   // For debug
...
void CheckUartIncoming(void)
    ...
    if (disable_host)
        ...
        pass_up_to_host = 1;                        // For next restart
        return;
        }
    ...
    while (rx_buffers_used > 0)
        ...
        if (((!info->connected || !pass_up_to_host) && h->operation_code != OP_LOOPBACK_UART)
         || info->usb_init_header)
            {
            // Toss the buffer, nowhere to send it or it doesn't belong to the host yet
            rx_tail += RX_BUFFER_SIZE;
            if (rx_tail >= rx_buffer_limit)
                rx_tail = uart_rx_buffer;
            irq = cpu_irq_save();
        ...
        // Header crc8 has already been checked

        if (h->operation_code == OP_HASH && h->data_length == 15)      // XXX
            {
            h->data_length--;                           // Drop the spare3 field
            h->crc8 = hf_crc8((uint8_t *)h);            // Fix up header crc
            }
        nb = h->data_length << 2;
        nb = (nb) ? nb + 8 : 8;                         // Drop the CRC
        total_sent += nb;
...
__attribute__((__interrupt__)) static void pdca_rx_int_handler(void)
    ...
    // doing this profile appears to result in data loss; probably normal
    // as the profileEnter() delays the setup of the next dma and
    // there's only the one byte holding register
...
__attribute__((__interrupt__)) static void usart_interrupt(void)
    ...
    // XXX profiling this function seems to result in a crash.  it could
    // certainly be that this overhead is too large so we drop data, which
    // could prevent the system from operating, but it shouldn't crash.
    // look into...
        ...
        if (b_error)
            ...
            if ((char)value == HF_PREAMBLE)
                ...
                if (rx_buffers_used >= RX_BUFFERS)
                    ...
                    // A candidate frame. Switch to DMA mode - 7 bytes remaining!
...
static void dma_RX_data_done(uint32_t status)
    ...
    if (status & PDCA_TRANSFER_COMPLETE)
        ...
        if (rx_buffers_used >= RX_BUFFERS)
            {
            // We're full. Gotta stop. Should never happen, uart (slow) -> USB (fast).
    ...
    if (status & PDCA_TRANSFER_COMPLETE)
        ...
        if (*rx_head == HF_PREAMBLE && crc8 == *(rx_head+7))
            ...
            if (dlen && dlen <= (RX_BUFFER_SIZE - 8 - 4) / 4)
                {
                // Set up DMA for the packet body
            ...
            {
            // Bad header CRC. Re-arm interrupt, look for a new preamble byte
...
//
// Sets up most of the parameters in the receive DMA channel. Called once at initialization time
//
...
void uart_open()
    ...
    if (hf_nvram_die_settings_valid())
        {
        // All boards start out at 230,400 baud. This is the only default common baud
        // rate available between 25 Mhz and 125 Mhz reference clock systems.
        // At a later stage, dynamic baud rate of 1.25 Mbaud is set
        info->ref_frequency = hf_nvram_die_settings()->ref_frequency;
        switch (info->ref_frequency)
            {
            case 125:
                usart_options.baudrate = BAUD_RATE_PWRUP_4;         // 230,400
                info->asic_baud_rate_code = 4;
                info->dynamic_baud_rate = 1250000;                  // Baud delay = 100 clock periods @ 125 Mhz
                break;

            case 25:
                usart_options.baudrate = BAUD_RATE_PWRUP_7 / 5;     // 1,152,000 / 5 = 230,400
                info->asic_baud_rate_code = 7;
                info->dynamic_baud_rate = 1250000;                  // Baud delay = 20 clock periods @ 25 Mhz
                break;

            default:
                // XXX Something new? Mixed chaining won't work but otherwise should be OK
        ...
        {
        // XXX Obsolete - die settings are always initialized now.
