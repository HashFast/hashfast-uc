...
#define FIRMWARE_VERSION       0x0000   // 0.0  Development firmware.  Set to real value on release build only.
...
// For all the uses of MIN, 125 is OK even if ref freq is 25
...
// And enable these before release candidates:
...
// whenever we add encryption/authentication, make sure to turn the CLI
// off for any builds to be released to customers, or restrict what the
// CLI can do to avoid creating backdoors
#define FEATURE_DEBUG_CLI

// collect code timing info; turn off for release builds since it would
// be adding some overhead with no benefit
//#define FEATURE_PROFILE

//
// Don't turn this on any more, because it means clock rate changes won't work
// It also makes things blow up because cgminer sends the extra work immediately,
// so we get work overruns and everything breaks down.
...
// Crude debugging. Comment out the function prototype and uncomment the #define
// to compile out all uprintf's.
#define uprintf(...)
